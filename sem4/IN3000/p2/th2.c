...
static void print_counter(int thread, int status) {
  ...
  if (thread == 2) {
    scrprintf(PRINT_LINE + 2, 0, "Thread  2 (lock)      : ");
  }
  else {
    scrprintf(PRINT_LINE + 3, 0, "Thread  3 (lock)      : ");
  }
  delay(DELAY_VAL);
  switch (status) {
    case RUNNING:
      if (thread == 2) {
      	scrprintf(PRINT_LINE + 2, 24, "%d", counter2++);
      } else {
      	scrprintf(PRINT_LINE + 3, 24, "%d", counter3++);
      }
      break;
    case DONE:
      scrprintf(line, 24, "Exited");
      break;
    case OK:
      scrprintf(line, 24, "Passed");
      break;
    default:
      scrprintf(line, 24, "Failed");
      break;
  }
}
